...
## ★A/B 백테스트 규율 (2026-08-07 25런 라운드 교훈)
- **인풋 게이트 원칙은 나 자신에게도 적용된다.** 수리든 신기능이든 게이트 없이 넣으면 회귀 시
  범인 분리가 불가능해진다(8/6에 Q2·Q3·Q10을 무게이트로 넣어 매트릭스 7런을 통째로 버렸다).
- **매 매트릭스의 첫 런은 "전부 OFF"로 이전 결과 재현 확인.** 오염된 베이스라인 위의 순위는 이전되지 않는다
  (1차에서 1위였던 ANCHOR가 깨끗한 기준선에선 −14,089로 뒤집혔다).
- **평온 창은 판별력이 0이다.** 6월 창에서 3구성이 소수점까지 동일했다. A/B 창은 스트레스 구간 필수.
- **"효과 0" ≠ "무해".** 발동 조건이 창에 없었을 뿐일 수 있다 → "미검증"으로 분류하고 게이트 유지.
- **손계산 권고는 발동 분포 가정을 명시할 것.** ANCHOR r=0.0875는 "ARM이 1.73랏에서 일어난다"는
  가정에 의존했고 실측과 달라 방패가 과대 생성됐다.
- 헤드리스 자동 실행: `terminal64.exe /config:<ini>` (`ShutdownTerminal=1`). 벤티지 창은 닫혀 있어야 함.
...
## 진행 스냅샷 (2026-08-07 r14 — 갱신할 것)
- 마스터: `src/mt5/GoldMiner_v5_5_5_internal_20260805.mq5` (r14, 벤티지 컴파일 0/0)
- **현장 배포 정본 프리셋: `GM555_FIELD_V2_20260806.set`** (E1+F1+FAR ON / C1·P1 OFF / 8-6 수리 게이트 전부 OFF) — 25런 검증 1위
- 8/6 헷지 수리(Q1~Q10)는 전부 인풋 게이트로 탑재·기본 OFF. 스트레스 창에서 Q1/Q2/Q3 열세, Q5/Q6/Q10 미발동(미검증)
