...
def process_file(path: Path, repo: Path) -> dict:
    ...
    trade_text = ""
    if not trades.empty:
        for column in ("symbol", "comment", "side"):
            if column in trades.columns:
                trade_text += " " + " ".join(trades[column].dropna().astype(str).head(200).tolist())
    # Put parsed symbols/comments first because the raw HTML header can be
    # larger than the classifier preview and may not contain the symbol.
    category = classify(path, trade_text + " " + text)
